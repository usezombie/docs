...
| Code | Meaning | Remediation |
|------|---------|-------------|
| `UZ-STARTUP-001` | Bootstrap failed | Generic startup failure. Check the preceding log lines for the underlying cause (sandbox prerequisites, config, etc.). |
| `UZ-STARTUP-002` | Config load failed | Required environment variables are missing or invalid. |
