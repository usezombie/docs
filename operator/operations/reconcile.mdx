...
description: "Dead-letter reconciliation for stale runs."
---
...
- **Stuck runs** — Runs that remain in a non-terminal state (`QUEUED`, `RUNNING`) beyond the expected timeout window. These can occur when a worker crashes mid-execution and no other worker reclaims the work.
