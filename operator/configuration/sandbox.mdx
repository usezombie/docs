...
description: "Resource limits and isolation policies."
---
...
The executor creates a cgroups v2 memory scope for each agent execution. If the agent exceeds the memory limit, the kernel OOM-kills the process. The executor detects OOM events and records them via the `oom_kills_total` metric.
