...
Each run has a lease — a bounded time window during which the worker holds exclusive ownership. If the worker crashes or becomes unresponsive, the lease eventually expires, and the in-process executor lease manager reaps the orphaned session and marks the run as failed.
