...
<Steps>
  ...
  <Step title="Store credentials — once">
    Add API keys, tokens, and secrets to the workspace credential store via `zombiectl secrets add` or the dashboard. Credentials are encrypted at rest and never passed into the sandbox.
  </Step>
...
| Kill switch | Manual stop from CLI or dashboard at any time |
