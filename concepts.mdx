...
<AccordionGroup>
  ...
  <Accordion title="ZombieConfig (SKILL.md + TRIGGER.md)">
    ...
    gather evidence from your hosting provider, your data-plane, and the failed CI workflow. Post an
    evidenced diagnosis to #platform-ops. Read-only outside the Slack post.
    ```

    See [Authoring a zombie](/zombies/authoring) for the full `TRIGGER.md` reference.
  </Accordion>
  ...
  <Accordion title="Tool">
    ...
    Constraint policy (e.g. "read-only against your infrastructure; the only write path is the channel post") lives as prose in `SKILL.md`. The prose is the contract today; structural enforcement comes when a second zombie reuses the same policy.
  </Accordion>
