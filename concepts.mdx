...
<AccordionGroup>
  ...
  <Accordion title="ZombieConfig (SKILL.md + TRIGGER.md)">
    ...
    gather evidence from Fly, Upstash, and the failed GitHub run. Post an
  ...
  <Accordion title="Trigger">
    How a zombie receives work. Three kinds, one reasoning loop:

    - **`webhook`** — an external system (GitHub Actions, Zendesk, custom) POSTs to `https://api.usezombie.com/v1/webhooks/{zombie_id}`. Signed with the workspace credential's secret.
    - **`cron`** — the scheduler fires on a cron expression. Useful for periodic health checks.
    - **`steer`** — a human invokes `zombiectl steer <zombie_id> "..."` (or sends a Slack DM). Same reasoning loop, manual prompt.

    Each event lands on `core.zombie_events` with actor provenance: `webhook:github`, `cron:<schedule>`, `steer:<user>`. The zombie doesn't branch on actor type — `SKILL.md` decides what to do.
  </Accordion>
  ...
  <Accordion title="Tool">
    ...
    - `memory_store` / `memory_recall` — persist a snapshot keyed by string.
    - `cron_add` / `cron_remove` — schedule future stage triggers.
    - `slack_post` — post to a configured Slack channel.

    Constraint policy (e.g. "only `kubectl get`, never `delete`") lives as prose in `SKILL.md`. The prose is the contract today; structural enforcement comes when a second zombie reuses the same policy.
  </Accordion>
  ...
  <Accordion title="Memory checkpoint">
    A durable snapshot the agent writes to during a long stage via `memory_store`. Layer 1 of the [context lifecycle](/concepts/context-lifecycle) — even if the rolling tool window drops old results from the active context, the checkpoint survives in `core.zombie_memories` and is recoverable via `memory_recall`.
  </Accordion>

  <Accordion title="Stage chunking">
    The failsafe when the active context approaches the model's working-memory cap. The agent writes a final snapshot, exits the stage with `exit_ok: false`, and the worker re-enqueues the same event chain as a fresh stage. The new stage starts clean and immediately calls `memory_recall` to load the snapshot. See [context lifecycle](/concepts/context-lifecycle) for the full picture.
  </Accordion>
