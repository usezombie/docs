...
<AccordionGroup>
  ...
  <Accordion title="Trigger">
    ...
    - **Cron firings** — the zombie schedules its own future runs via the `cron_add` tool. The runtime fires them on schedule and emits events with `actor=cron:<schedule>`.
