...
<AccordionGroup>
  ...
  <Accordion title="Run">
    A single execution of a spec. Each run has a lifecycle state: `PLANNED`, `RUNNING`, `COMPLETED`, or `FAILED`. A completed run produces artifacts (the implementation diff, gate logs) and a scorecard. Runs are immutable once finished — to retry, create a new run.
  </Accordion>
