...
<AccordionGroup>
  ...
  <Accordion title="Skill">
    A named tool available to a zombie's agent — the only way agents interact with external
    services. Skills are declared in `TRIGGER.md` as bare tool names; the runtime injects the
    credential and executes the call outside the sandbox. Examples: `kubectl`, `docker`,
    `github`, `slack`.
