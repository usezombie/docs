...
| `UZ-WH-020` | 401 | Webhook credential not configured | No webhook credential is configured for this zombie's source. Run `zombiectl credential add <source> --data=@-` with `{ "webhook_secret": "..." }` in the zombie's workspace, then resend. |
...
| Code | HTTP | Title | Common Causes |
|---|---|---|---|
| `UZ-TOOL-004` | 400 | Tool not attached | The tool isn't in this zombie's `tools:` list. Add it to `TRIGGER.md` `x-usezombie.tools:` and re-install. |
| `UZ-TOOL-005` | 400 | Unknown tool | Tool name not recognized — typo against the [Tools catalogue](/zombies/tools) is the usual cause. |
