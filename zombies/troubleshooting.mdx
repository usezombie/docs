...
**Local validation tip:** the parser the platform uses is the same one `zombiectl install --from` runs. There's no separate lint command. If you want to dry-run, `zombiectl install --from <path> --json` returns the validation outcome without doing anything irreversible (the call still uploads on success — there is no `--validate-only` flag).
...
| `UZ-WH-020` | 400 | `secret_ref` in `TRIGGER.md` references a vault key that doesn't exist. Run `zombiectl credential list` to confirm. |
| `UZ-WH-030` | 413 | Payload exceeds the size limit. The upstream is sending too much; configure a slimmer event payload on their side. |
...
| `UZ-TOOL-005` | Unknown tool | `tools:` in `TRIGGER.md` lists a name not in the catalogue. Check spelling against [Tools catalogue](/zombies/tools). |
