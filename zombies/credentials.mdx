...
The **workspace vault** is the answer. Secrets live in a dedicated, encrypted vault scoped to the workspace. Each secret is addressed by a bare key name (e.g. `github`, `slack`) and stores a JSON object with the provider's fields. Your zombie's `TRIGGER.md` declares which key names it needs; at event time the credential firewall fetches the value, makes the outbound call via the `http_request` tool, and returns only the response. The raw value never crosses the sandbox boundary.
...
sequenceDiagram
    ...
    A->>F: http_request POST api.github.com/...
    F->>V: lookup(workspace_id, "github")
    V-->>F: { api_token, webhook_secret }
    F->>E: POST api.github.com/...<br/>Authorization: Bearer <api_token>
    E-->>F: 200 OK
