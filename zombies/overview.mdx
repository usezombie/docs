...
2. **Processing.** When a signed event arrives on the webhook (or a cron fires, or a steer message lands), the event loop claims it, opens a stage in the sandboxed agent, and the agent reasons, invokes tools, and produces a result. Stages process sequentially per zombie; the event stream on `core.zombie_events` is the durable record.
