...
```
homelab/
├── SKILL.md         # "You are a homelab diagnostic agent..." + the read-only policy in prose
└── TRIGGER.md       # webhook trigger, tools=[kubectl, docker, ssh], credential refs, budget
```

- **Tools:** `kubectl`, `docker`, `ssh`. Read-only policy lives as prose in `SKILL.md`.
- **Credentials it expects:** `kube_<name>`, optionally `ssh_<name>`.
- **Worker placement:** inside your homelab — kubeconfig never leaves your network.
