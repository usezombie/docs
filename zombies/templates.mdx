...
└── TRIGGER.md       # webhook trigger, source=email, skills=[agentmail]
```

- **Trigger:** webhook, `source: email`
- **Skills:** `agentmail`
- **Credentials it expects:** `agentmail_api_key`
- **Approval gate:** optional — on `agentmail.reply`
...
└── TRIGGER.md       # webhook trigger, source=slack, skills=[slack, github]
```

- **Trigger:** webhook, `source: slack`
- **Skills:** `slack`, `github`
- **Credentials it expects:** `slack_bot_token`, `github_token`
- **Approval gate:** recommended — on `github.open_pr`
...
```markdown SKILL.md
You are a simple HTTP echo agent. When an event arrives, log its `type` field
