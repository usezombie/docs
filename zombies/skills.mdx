...
The `tools:` list is the explicit allowlist of platform-catalog tools the agent may invoke (`agentmail`, `slack`, `git`, `github`, `linear`, `jira`, `grafana`, `clerk`, generic `http`, plus the built-in `memory_*` and `cron_*` tools). A compromised or jailbroken agent cannot reach outside this list.

### Credentials

Each entry under `credentials:` is a **bare vault key name** — no URI scheme, no path. The firewall resolves the name at tool-call time; the agent sees the tool's response, never the raw secret. Add and rotate via [Credentials](/zombies/credentials).
...
| `UnknownRuntimeKey` at install | Typo in a runtime subkey (e.g. `contxt:` instead of nothing — drop it) | Fix the spelling, or remove the key. Validation under `x-usezombie:` is strict. |
