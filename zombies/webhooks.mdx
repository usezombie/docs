...
The approval webhook carries the approval decision back into the event loop. The zombie's declared approval token (in its `TRIGGER.md` under `approval.token_ref`) authenticates the call.
...
A companion `/grant-approval` endpoint covers the grant-flow case where a zombie asks for permission to attach a new integration (for example, a new Slack workspace). See [Skills and approvals](/zombies/skills#approvals) for when to declare an approval gate.
