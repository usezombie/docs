...
`trigger.source` is a human-readable label (`slack`, `github`, `linear`, `grafana`, ...) that appears in the activity stream; the platform routes strictly by the zombie ID in the URL path. The `signature` block declares how the platform will authenticate incoming requests.
