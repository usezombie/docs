...
Authored as one `SKILL.md` + one `TRIGGER.md`. The tools the zombie can invoke — `docker`, `kubectl`, `tls-probe` — are named in `TRIGGER.md` as bare tool names. The read-only policy is prose inside the `SKILL.md` prompt: **"enumerate only; never change configs, never update images, never rotate secrets; stop at the report."**
