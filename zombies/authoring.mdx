...
|---|---|---|
| `trigger.type` | yes | One of `webhook` \| `cron` \| `api` \| `chain`. |
| `trigger.source` | per type | Required for `webhook` and `chain`. |
...
| `tools` | yes | Non-empty list of tool names from the platform catalog. |
| `credentials` | optional | Vault key names the zombie reads at event time. |
| `network.allow` | yes | Outbound hostname allowlist enforced by the credential firewall. |
| `budget.daily_dollars` | yes | Rolling 24-hour dollar ceiling. |
| `budget.monthly_dollars` | optional | Calendar-month dollar ceiling. |

### Tools

The `tools:` list is the explicit allowlist of tools the agent may invoke. Real tool names today:

- **`http_request`** — outbound HTTP calls. The credential firewall substitutes secrets in headers and bodies at the sandbox boundary; the agent never sees raw bytes. Slack posts, GitHub API calls, and any provider-specific endpoint all go through this one tool.
- **`memory_store` / `memory_recall` / `memory_list` / `memory_forget`** — durable cross-event learning. See [Memory](/memory).
- **`cron_add` / `cron_list` / `cron_remove`** — schedule future stage triggers from inside the agent loop.

A compromised or jailbroken agent cannot reach outside this list.

### Credentials

Each entry under `credentials:` is a **bare vault key name** referencing a JSON-shaped secret in the workspace vault (e.g. `github` carrying `{api_token, webhook_secret}`, `slack` carrying `{bot_token}`). Names like `github`, `slack`, `linear`, `jira` are credential references — **not** tool names. The firewall resolves the named credential at `http_request` time and substitutes its fields into outbound calls; the agent sees the response, never the raw secret. Add and rotate via [Credentials](/zombies/credentials).
