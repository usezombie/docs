...
## Path-based install: the flagship `platform-ops` sample

The `platform-ops` zombie ships in the `usezombie/usezombie` repo at `samples/platform-ops/` rather than as a bundled template. Install it by path:

```bash
zombiectl install --from samples/platform-ops
```

What it does: wakes on a GitHub Actions deploy-failure webhook, gathers evidence from the failed workflow's logs, your hosting provider, and your data-plane, then posts an evidenced diagnosis to Slack. Read-only against GitHub, Fly, and Upstash; the only write path is the Slack post. Required credentials: `github`, `fly`, `upstash`, `slack`. See [zombie templates](/zombies/templates) for the full walkthrough.
