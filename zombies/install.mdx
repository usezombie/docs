...
| `homelab` | The flagship v2 sample — read-only `kubectl` / Docker diagnosis. See [Homelab Zombie](/zombies/homelab). | `kubectl`, `docker`, `ssh` (read-only policy in `SKILL.md`) |

Run `zombiectl install` with no argument to see the current bundled set in the error output.

For design references (`homebox-audit`, `migration-zombie`, `side-project-resurrector`) that are not yet bundled install targets, see the [zombies overview](/zombies/overview).
