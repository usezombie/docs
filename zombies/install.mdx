...
The `name` field becomes the zombie's display name. The `trigger.source` field is a human-readable label that appears in logs — routing is always by zombie ID, so two zombies with `source: email` do not conflict.
...
1. **Edit the files**, then `zombiectl up` to deploy as-is. The bundled templates reference credentials via `op://` placeholders; the platform resolves these against your workspace vault.
2. **Add credentials first** via `zombiectl credential add`, then `zombiectl up`. Either order works — the zombie will not process events until every credential it references is present in the vault.
