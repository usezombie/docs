...
A zombie does its work in **stages** — one stage is one end-to-end run on one trigger: event arrives → agent reasons → tool calls → result. Most zombies finish a stage in seconds.
...
<Columns cols={2}>
  ...
  <Card title="Long incidents keep going" icon="layer-group">
    If one run hits the model's context window, the platform restarts it on a fresh prompt with prior findings recovered. <a href="/concepts/context-lifecycle">How long does a zombie run? →</a>
  </Card>
  ...
  <Card title="Kill switch" icon="power-off">
    `zombiectl kill` stops the run cleanly; nothing on the activity stream is lost. Resume via `zombiectl steer`.
  </Card>
</Columns>
