...
<Update label="May 6, 2026" tags={["Bug fixes", "Security", "Internal"]}>
  ...
  Both now apply the same placeholder substitution the tool-arg path has always used (`${secrets.llm.api_key}`, `${secrets.github.token}`). Operators watching the activity stream or pub/sub subscribers no longer see resolved secret bytes for any frame variant.
