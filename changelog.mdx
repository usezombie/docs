...
<Update label="May 4, 2026" tags={["Breaking", "API", "CLI"]}>
  ...
  - **Operator-minimum role now applies to *every* status transition** (RULE BIL). Previously only the retired `DELETE /current-run` enforced it. Existing callers using workspace-member role for `PATCH {status: "killed"}` will now receive `403 UZ-AUTH-005`. Pure `config_json` patches (no `status` field) still permit workspace-member role.
