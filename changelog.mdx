...
<Update label="Apr 22, 2026" tags={["What's new", "UI", "CLI", "Integrations"]}>
  ...
  - **New operator quickstart:** `/operator/quickstart` — self-hosted status with worker placement in your own network and `op://` vault integration for credential provisioning. A dedicated "Self-hosting" group now collects every operator page in the sidebar.
