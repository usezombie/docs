...
<Update label="Apr 22, 2026" tags={["What's new", "Breaking", "UI", "API", "CLI"]}>
  ...
  **Renamed (breaking):**
  ...
  - `zombiectl zombie list [--workspace-id ID] [--cursor C] [--limit N] [--json]` — new subcommand that mirrors the `/zombies` list in Mission Control (cursor-paginated, honours the same `limit ≤ 100` clamp).
