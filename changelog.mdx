...
<Update label="v0.21.0 — Apr 18, 2026" tags={["Docs"]}>
  ...
  A new [Zombies](/zombies/overview) section walks through installing a template, adding credentials, running, observing, and killing a zombie. Pages describing the old spec/run workflow have been retired; the old `/specs/*` and `/runs/*` URLs now 404.
...
<Update label="v0.14.0 — Apr 12, 2026" tags={["New", "Memory", "API"]}>
  ...
  Zombies remember facts across runs. Memory is row-scoped per zombie and persists in Postgres — a lead-collector zombie doesn't re-research the same lead, a support zombie doesn't re-ask customers their plan. Tools: `memory_store`, `memory_recall`, `memory_list`, `memory_forget`.
</Update>
