...
<Update label="Apr 29, 2026" tags={["What's new", "API", "UI"]}>
  ...
  - **Slack and dashboard parity.** Slack callbacks and dashboard clicks now share one resolve core. The schema-level append-only trigger plus the `WHERE status='pending'` precondition give at-most-one-resolution guarantees — the loser sees 409 with the original outcome and resolver attribution, never silent overwrite.
  ...
  - `ApprovalGate` shape includes the new dashboard-visible fields (`gate_kind`, `proposed_action`, `evidence` as JSONB, `blast_radius`, `timeout_at`, `resolved_by`) on top of the existing audit fields.

  ## Bug fixes

  - **Slack and dashboard race no longer overwrites silently.** Before this release, the Slack callback wrote the Redis decision key directly without a DB precondition; a dashboard click that happened to land first could be overwritten by the Slack click moments later. Both paths now go through the same DB UPDATE WHERE status='pending' atomic transition; the loser observes 409 with the original outcome.
</Update>
