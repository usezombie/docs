...
<Update label="Apr 27, 2026" tags={["Internal"]}>
  ## Tightened internal Zig surface area

  An audit pass narrowed `pub` visibility on 355 symbols across the Zig backend that were not referenced outside their declaring file, and added explicit caller-owned-allocator doc-comments to 21 heap-owning structs whose ownership was implicit in their `deinit` signatures. No user-visible API or CLI behavior changes.
</Update>
...
<Update label="Apr 22, 2026" tags={["What's new", "UI", "CLI", "Integrations"]}>
  ...
  - **New CLI reference:** `/cli/zombie-lifecycle` — compact reference for `install`, `up`, `status`, `logs`, `kill`, `credential add` with copyable examples.
  - **Self-hosting status page:** `/operator/quickstart` — an honest map of what ships today (systemd `zombied worker` on bare-metal, Zig API binary, local-dev compose for Postgres + Redis) vs. what's on the roadmap (containerized worker, `zombiectl worker token` lifecycle). A dedicated "Self-hosting" group now collects every operator page in the sidebar.
  - **Four new zombie pages** under `/zombies/`:
    - `/zombies/homelab` — the flagship Homelab Zombie (`kubectl` / Docker diagnosis without holding the kubeconfig).
    - `/zombies/homebox-audit` — quarterly self-hosted-stack audit.
    - `/zombies/migration-zombie` — mechanical codebase migrations.
    - `/zombies/side-project-resurrector` — revive a dormant side-project repo.
...
<Update label="Apr 11, 2026" tags={["Internal"]}>
