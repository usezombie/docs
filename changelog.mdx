...
<Update label="May 3, 2026" tags={["Feature", "CLI", "Integrations"]}>
  ...
  The previously-aspirational `x-usezombie.context` and `x-usezombie.model` shape from the platform-ops sample is now accepted by the runtime parser, plumbed through `ZombieConfig` into the executor's `ContextBudget`. Frontmatter overrides take precedence; absent or zero fields fall through to runtime auto-defaults via `applyContextDefaults`.
</Update>
