...
<Update label="May 4, 2026" tags={["Breaking", "API", "Pricing", "Marketing"]}>
  ...
  - **FAQ context-window answer rewritten to match the runtime.** Previous copy claimed "the runtime layers three independent mechanisms" — that misattributed enforcement. The truth: the runtime emits three signals (tool-result window, memory checkpoints, stage-chunk threshold), the agent enforces via `memory_store(category='conversation', ...)`, the worker re-enqueues stages on a continuation chain capped at 10, and nullclaw runs its own rolling-summary compaction underneath. Page anchored to actual file:line references at `concepts/context-lifecycle`.
