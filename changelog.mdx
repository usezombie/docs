...
<Update label="Apr 23, 2026" tags={["Breaking", "What's new", "CLI"]}>
  ## One command installs a zombie

  `zombiectl install --from <path>` is now the only way to install. It reads `SKILL.md` and `TRIGGER.md` from the directory you point at, registers the zombie, and the zombie is live before the command returns — no separate start step, no follow-up `zombiectl up`. The output is deliberately minimal:
  ...
  Use `zombiectl status` or `zombiectl list` to fetch the webhook URL when you need it for external wiring. In scripts, `--json` still returns `zombie_id`, `webhook_url`, and `name` in one object.

  ## Upgrading

  - **`zombiectl up` is removed.** It used to POST the zombie dir in cwd to the server. That step is now part of `install --from`. Replace `zombiectl install <template>` followed by edits followed by `zombiectl up` with a single `zombiectl install --from <path>` that points at a directory you authored directly. Any script hitting `zombiectl up` now gets the unknown-command branch.
  - **Bundled templates (`zombiectl install lead-collector` / `slack-bug-fixer`) are removed.** The `zombiectl/templates/` directory no longer ships. Samples in `samples/` (e.g. `samples/homelab`) are the new starting point — clone one, edit, install.
  - **Install no longer prints the webhook URL in pretty mode.** If your flow piped the install output to grab the URL, switch to `zombiectl status --json | jq -r '.zombies[] | select(.name=="<name>") | .webhook_url'` (or keep using `--json` on install and read `.webhook_url` from the JSON response).

  ## CLI

  - `zombiectl install --from <path>` — registers the zombie at the given path. `SKILL.md` and `TRIGGER.md` are required. Typed errors on missing path / missing file: `ERR_PATH_NOT_FOUND`, `ERR_SKILL_MISSING`, `ERR_TRIGGER_MISSING`. Exit 2 for usage errors (`zombiectl install` with no flag; legacy `zombiectl install <name>` without `--from`).
  - `zombiectl status` empty-list hint now points at `zombiectl install --from <path>` instead of the old `<template>` form.

</Update>

<Update label="Apr 22, 2026" tags={["What's new", "Breaking", "UI", "API", "CLI"]}>
  ## Mission Control dashboard — full lifecycle in the browser; kill switch moves to DELETE
  ...
  Credit exhaustion is now operator-visible in the dashboard. When a tenant's balance hits zero, a destructive banner appears above the zombies list and a "Balance exhausted" badge renders on each zombie's detail page — both driven by the `is_exhausted` / `exhausted_at` fields on `GET /v1/tenants/me/billing`.
  ...
  **Renamed (breaking):**
  ...
  - `zombiectl --help` now surfaces the full `zombie` command group — `install | up | status | kill | logs | credential` — alongside the existing `login / workspace / specs / doctor` commands.
  - `zombiectl zombie list [--workspace-id ID] [--cursor C] [--limit N] [--json]` — new subcommand that mirrors the `/zombies` list in the dashboard (cursor-paginated, honours the same `limit ≤ 100` clamp).
  - `zombiectl workspace show [--workspace-id ID]` — new subcommand that mirrors the `/settings` page (prints workspace ID, name, and active-workspace status).
  - **Active workspace is now persistent.** `zombiectl workspace use <workspace_id>` writes it to `~/.config/zombiectl/config.json`; subsequent commands (`zombie list`, `zombie status`, `workspace show`, etc.) default to it when `--workspace-id` is omitted. The UI's `active_workspace_id` cookie and the CLI's config file stay independent — setting one doesn't affect the other.
  - `zombiectl zombie up` still prints the `🎉 Woohoo! Your zombie is installed and ready to run.` success line (unchanged).
  - `zombiectl zombie kill` is unaffected by the kill-switch path rename — it continues to call `DELETE /zombies/{id}` (full delete, not the current-run kill) as it always did.
</Update>
