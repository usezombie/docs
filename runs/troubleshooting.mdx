...
<AccordionGroup>
  ...
  <Accordion title="Credit exhaustion">
    ...
    **Fix:**
    - Check your current balance in the [dashboard](https://app.usezombie.com/billing).
  ...
  <Accordion title="Duplicate spec">
    ...
    **Cause:** The same spec content, repository, and base commit SHA already has an active run (status `QUEUED` or `RUNNING`).
...
Replay the complete event history of a finished run:

```bash
zombiectl runs replay <id>
```

The replay streams every gate result, loop iteration, and captured output as Server-Sent Events — useful for debugging failed runs or reviewing what happened at each step.

You can also replay via the API:
