...
2. A run row is written to the database with status `PLANNED`.
3. The run is enqueued in Redis for the next available worker to pick up.
...
| State | Meaning |
| --- | --- |
| `PLANNED` | Run accepted, queued for execution |
