...
The original tool results stay on `core.zombie_events` (durable forever) but leave the active model prompt — subsequent reasoning sees the summary instead. This is the *enforcement primitive* the runtime relies on for its "rolling tool-result window" framing: the runtime emits log signals at a tunable cadence; the agent's `SKILL.md` prose responds by writing this kind of `conversation`-category entry.
